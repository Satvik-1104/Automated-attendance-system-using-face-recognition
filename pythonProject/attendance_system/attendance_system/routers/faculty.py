...
def faculty_has_access(faculty_id: int, section_id: int, course_code: str, db: Session):
    return db.query(ClassScheduleAccess).filter(
        and_(
            ClassScheduleAccess.faculty_id == faculty_id,
...
def request_otp(
    faculty: FacultyOTPRequest,
    background_tasks: BackgroundTasks,
    db: Session = Depends(get_db)
):
    valid_entry = db.query(ValidFaculty).filter(
        (ValidFaculty.employee_id == faculty.faculty_id) &  # Map correctly to employee_id
        (ValidFaculty.email == faculty.email) &
        ValidFaculty.is_verified  # Ensure verification
    ).first()

    if not valid_entry:
        raise HTTPException(status_code=400, detail="Invalid faculty ID or email")

    existing_faculty = db.query(Faculty).filter(
        (Faculty.employee_id == faculty.faculty_id) |
    ...
    existing_otp = db.query(OTPVerification).filter(
        OTPVerification.email == faculty.email,
        OTPVerification.user_type == "faculty"
    ).first()
    ...
    background_tasks.add_task(send_email, faculty.email, f"Your OTP for faculty registration is {otp_code}")
...
def register_faculty(faculty: FacultyRegister, db: Session = Depends(get_db)):
    valid_entry = db.query(ValidFaculty).filter(
        ValidFaculty.employee_id == faculty.faculty_id,
        ValidFaculty.email == faculty.email
    ).first()
    if not valid_entry:
        raise HTTPException(status_code=400, detail="Invalid faculty ID or email")
    ...
    verified_otp.faculty_id = new_faculty.employee_id
    db.delete(verified_otp)
    db.commit()

    for assignment in faculty.teaching_assignments:
        batch = assignment.batch
        for course_code in assignment.course_code:  # Loop over course_codes
            for section_name in assignment.sections:
                ...
                for schedule in schedules:
                    access = ClassScheduleAccess(
                        ...
                        faculty_id=new_faculty.employee_id
                    )
                    db.add(access)

    db.commit()

    return {"message": "Faculty registered successfully", "faculty_id": new_faculty.employee_id}
...
def mark_attendance(
    ...
    if not faculty_has_access(current_faculty.employee_id, request.section_id, request.course_code, db):
    ...
    existing_record = db.query(AttendanceReport).filter(
        ...
        AttendanceReport.section_id == request.section_id,  # Added
        AttendanceReport.course_code == request.course_code  # Added
    ).first()

    if existing_record:
        existing_record.is_absent = not request.is_present
        existing_record.marked_by_faculty = current_faculty.employee_id
    else:
        new_record = AttendanceReport(
            roll_number=request.roll_number,
            class_time=request.class_time,
            is_absent=not request.is_present,
            marked_by_faculty=current_faculty.employee_id,
...
def view_attendance(
    ...
    if not faculty_has_access(current_faculty.employee_id, section_id, course_code, db):
    ...
    for roll_number, records in grouped.items():
        ...
        response.append({
            ...
            "roll_number": roll_number,  # Added roll_number
...
@router.get("/pending_corrections", operation_id="get_faculty_pending_corrections")
def get_pending_corrections(
        faculty: Faculty = Depends(get_current_faculty),
        db: Session = Depends(get_db)
):
    faculty_sections = db.query(ClassSchedule.section_id, ClassSchedule.course_code).join(ClassScheduleAccess).filter(
        ClassScheduleAccess.faculty_id == faculty.employee_id  # type: ignore
    ).all()
    faculty_section_ids = [section.section_id for section in faculty_sections]
    faculty_course_codes = [section.course_code for section in faculty_sections]

    pending_requests = db.query(AttendanceCorrection).filter(
        AttendanceCorrection.section_id.in_(faculty_section_ids),
        AttendanceCorrection.course_code.in_(faculty_course_codes),
...
def approve_correction(
    ...
    if not faculty_has_access(
        current_faculty.employee_id,
    ...
    correction_request.processed_by_faculty = current_faculty.employee_id
    ...
    if request.approval_status:
        ...
        if attendance_record:
            ...
            attendance_record.marked_by_faculty = current_faculty.employee_id
...
def bulk_approve_correction(
    ...
    faculty_sections = db.query(ClassSchedule.section_id, ClassSchedule.course_code).join(ClassScheduleAccess).filter(
        ClassScheduleAccess.faculty_id == current_faculty.employee_id  # type: ignore
    ).all()

    faculty_section_ids = [s.section_id for s in faculty_sections]
    faculty_course_codes = [s.course_code for s in faculty_sections]
    ...
    for correction in corrections_to_approve:
        ...
        correction.processed_by_faculty = current_faculty.employee_id
...
def update_schedule(
    ...
    if request.update_type == "CANCELLED":
        dsu = DynamicScheduleUpdate(
            ...
            updated_by_faculty=current_faculty.employee_id
        )
        db.add(dsu)
        db.commit()
    ...
    attendance_details = [
        {
            ...
            "is_absent": record.is_absent,
            "marked_by_faculty": record.marked_by_faculty
        }
        for record in records
    ]
...
def get_class_schedules(
    ...
    """
    Returns all class schedules (all classes) that the current faculty has access to,
...
def get_attendance_summary(
    ...
    total_records = db.query(AttendanceReport).filter(
        AttendanceReport.marked_by_faculty == current_faculty.employee_id  # type: ignore
    ).count()
...
def get_assigned_sections(
    ...
    assigned to the current faculty.
    """
    schedules = db.query(ClassSchedule).join(ClassScheduleAccess).filter(
        ClassScheduleAccess.faculty_id == current_faculty.employee_id  # type: ignore
    ).all()
    ...
    unique_sections = {}
    for schedule in schedules:
        key = schedule.section_id  # Group by unique section_id
...
def get_attendance_records(
    ...
    """
    Returns all attendance records for sections the current faculty has access to.
    """
    faculty_sections = db.query(ClassSchedule.section_id, ClassSchedule.course_code).join(
        ClassScheduleAccess
    ).filter(
        ClassScheduleAccess.faculty_id == current_faculty.employee_id  # type: ignore
    ).all()
    ...
    records = db.query(AttendanceReport).filter(
        AttendanceReport.section_id.in_(faculty_section_ids),
        AttendanceReport.course_code.in_(faculty_course_codes)
    ).all()
