...
const VerifyOTP = ({ email, onSubmit }) => {
  ...
  const handleVerifyOTP = async () => {
    ...
    setLoading(true);
    try {
      // We're just verifying the OTP is correct here, not actually resetting the password yet
      onSubmit(otpValue);
    } catch (error) {
      setMessage("Invalid OTP: " + (error.response?.data?.detail || "Please try again"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResendOTP = async () => {
    ...
    setTimer(30);
    try {
      await axios.post(`${BASE_URL}/auth/forgot-password`, null, {
        params: { email }
      });
      ...
      setMessage("Failed to resend OTP: " + (error.response?.data?.detail || "Please try again"));
      setResendDisabled(false);
    }
  };
